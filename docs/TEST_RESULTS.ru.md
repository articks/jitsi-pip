...
- Vitest — 4 test files, 35 тестов успешно.
...
- `dist/jitsi-meet-pip.js` — 64 359 байт; SHA-256 `753d14c79c5172191c35c0db9b3e08a1da18c96cff22f6ac401171a948dc4ace`.
- `dist/jitsi-meet-pip.min.js` — 42 465 байт; SHA-256 `166af602fe5fa376ba04c4c04bc33617a2391427112278dd450605bf7a219e69`.
...
- Локальный Nginx кеширует файл из `/libs/` на год, поэтому `plugin.head.html` использует актуальный cache-busting URL `?v=1.2.11`; Jitsi отдаёт именно bundle `1.2.11`.
...
- Красная кнопка завершения звонка клонирует текущий залитый SVG из основной панели Jitsi и удаляет его фиксированные размеры; соответствие закреплено regression-тестом.
...
- Локальный HTTPS отвечает по HTTP/2, цепочка успешно проверена с созданным CA. Актуальная локальная сборка имеет версию `1.2.11`; её контрольные суммы перечислены в разделе автоматических проверок.
