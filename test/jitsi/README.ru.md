...
подключается как `jitsi-meet-pip.min.js?v=1.2.13`, потому что Nginx Jitsi кеширует
...
Перед переключением вкладки ожидаются `version === "1.2.13"`,
`protocolEligible === true`, `secureContext === true`,
`handlerRegistered === true`, `captureActive === true` и `ready === true`.

## Проверка окна PiP 1.2.13

1. Полностью перезагрузите комнату и убедитесь, что `JitsiBrowserPiP.version === "1.2.13"`.
...
8. При каждом открытии в Chrome 130+ ожидается портретный размер около `240×480`; браузер может скорректировать его под доступное место. Более старый Chromium может восстановить пользовательский размер окна.
