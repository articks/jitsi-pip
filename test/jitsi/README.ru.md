...
подключается как `jitsi-meet-pip.min.js?v=1.2.14`, потому что Nginx Jitsi кеширует
...
Перед переключением вкладки ожидаются `version === "1.2.14"`,
`protocolEligible === true`, `secureContext === true`,
`handlerRegistered === true`, `captureActive === true` и `ready === true`.

## Проверка окна PiP 1.2.14

1. Полностью перезагрузите комнату и убедитесь, что `JitsiBrowserPiP.version === "1.2.14"`.
2. Откройте PiP без демонстрации. Активный удалённый собеседник должен отображаться в большом верхнем блоке и отсутствовать в нижней сетке; при выключенной камере сверху показываются аватар или инициалы.
...
4. Запустите демонстрацию другого окна, всего экрана или вкладки — удалённую либо собственную. Демонстрация должна заменить крупного собеседника в верхней половине окна.
5. Проверьте в консоли `JitsiBrowserPiP.getState().screenShare`: для собственной демонстрации ожидается `local === true`.
6. Пока автор демонстрации говорит, его камера или аватар должны появляться в нижней сетке. Неактивный автор специально в сетку не добавляется.
7. Остановите демонстрацию. Desktop-трек должен отсоединиться, активный удалённый собеседник — вернуться в верхний блок и исчезнуть из нижней сетки без перестроения окна.
8. Проверьте нижнюю половину: четыре карточки должны образовать сетку `2×2`, а две — один ряд и заполнить всю высоту блока.
9. При каждом открытии в Chrome 130+ ожидается портретный размер около `240×480`; браузер может скорректировать его под доступное место. Более старый Chromium может восстановить пользовательский размер окна.
10. Включите и выключите микрофон и камеру из Jitsi и из PiP: включённое состояние должно показывать обычную иконку, выключенное — перечёркнутую.
11. Проверьте самую нижнюю строку `Участников: N · В лобби: M`: числа должны меняться без переоткрытия PiP при входе/выходе участника и при появлении/допуске пользователя из лобби.
12. Измените несколько полей подписей в `plugin.head.html`, полностью перезагрузите комнату и проверьте toolbar, заголовок PiP, подсказки кнопок, подпись демонстрации и строку счётчиков. Полный список полей приведён в корневом `README.md`.
