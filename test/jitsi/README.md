...
подключается как `jitsi-meet-pip.min.js?v=1.2.5`, потому что Nginx Jitsi кеширует
...
Перед переключением вкладки ожидаются `version === "1.2.5"`,
`protocolEligible === true`, `secureContext === true`,
`handlerRegistered === true`, `captureActive === true` и `ready === true`.

## Проверка окна PiP 1.2.5

1. Полностью перезагрузите комнату и убедитесь, что `JitsiBrowserPiP.version === "1.2.5"`.
