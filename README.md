...
```html
<script src="/libs/jitsi-meet-pip.min.js?v=1.1.4"></script>
```
...
- В Document Picture-in-Picture говорящие получают приоритет, а свободные места заполняются остальными подключёнными удалёнными участниками — максимум четыре плитки.
- Демонстрации экрана, shared video и локальный пользователь не добавляются.
- При выключенной камере отображаются имя и инициалы.
- Сообщение «Ждём активного собеседника» показывается только когда в конференции нет удалённых участников.
- Звук продолжает воспроизводиться только основным окном Jitsi.
- Доступны кнопки микрофона, камеры, возврата в конференцию и завершения звонка.
...
- В Safari и других браузерах без Document PiP используется стандартный Video PiP с одним говорящим и системными элементами управления.
