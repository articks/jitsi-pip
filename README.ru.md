...
| `waitingParticipantLabel` | `aria-label` свободной позиции нижней сетки |
| `noActiveSpeakerLabel` | `aria-label` пустого верхнего блока и текст Video PiP без участника |
| `noScreenShareLabel` | Сохранённое для обратной совместимости поле конфигурации |
...
- Область над кнопками всегда делится пополам: верхняя половина отведена выбранному медиа, нижняя — участникам. Четыре карточки заполняют нижнюю половину сеткой `2×2`, а две располагаются в один ряд и растягиваются на всю её высоту.
- Начальный размер Document PiP — `240×480`; Chrome 130+ получает `preferInitialWindowPlacement: true`, поэтому не восстанавливает старый размер при повторном открытии. Пользователь может изменять текущее окно вручную, а браузер вправе дополнительно ограничить размер доступным экраном.
- Активная демонстрация экрана отображается в большой верхней плитке. Поддерживаются удалённая и собственная демонстрации; отдельный desktop track не заменяет локальную карточку камеры внизу.
...
- Без активной демонстрации верхняя плитка показывает удалённого dominant speaker, затем недавнего active speaker или первого доступного удалённого участника. Этот участник исключается из нижней сетки до применения лимита в четыре карточки, поэтому освободившееся место занимает следующий участник, а не дубликат большой плитки.
- При активной демонстрации нижняя сетка возвращается к обычному выбору. Поэтому говорящий автор демонстрации может одновременно отображаться камерой или аватаром снизу, пока его экран показан сверху; неактивный автор специально в сетку не добавляется.
- Если удалённых участников нет, верхняя плитка показывает иконку участника без видимой подписи. Запуск и остановка демонстрации не меняют внешнюю компоновку окна.
...
`getState().participants` содержит только реальные карточки, выбранные для нижней
сетки. Участник, крупно показанный сверху, исключён из списка до появления демонстрации.
